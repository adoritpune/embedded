					...
					if(SPI2STATbits.SPITBF==0)
					{
						...
						*/
						...
						if((VOLTAGE<100)&&(para.mmamode.setcurrent >= ANTISTICKCURR))      //For MMA antistick operation //@sbm
						{
						   ...
						   daccount	=(sum_setcurrent*CCSLOPENEWANT)-CCOFFSETNEW ;//8.0;//CCSLOPE);//5.20);//490);//4.60);//4.70);//CCSLOPENEW
						   if(daccount>=4095)
						   daccount=4095;
						   SPI2BUF		=28672+daccount;
						...
						{
							antistick_f=0;							
							if(para.mmamode.setcurrent <= 75)//122)
							{								
								sum_setcurrent=para.mmamode.setcurrent+sck_current_ref;
							    if(sum_setcurrent>=500)
							    sum_setcurrent=500;							    
								daccount	=((sum_setcurrent*CCSLOPENEW_1STAG)-CCOFFSETNEW_1STAG);//+Recall_location;CCSLOPENEW_1STAG
								if(daccount>=4095)
								daccount=4095;
								SPI2BUF		=28672+daccount;								
							}
							else
							{
							    ...
							    if(sum_setcurrent>=500)
							    sum_setcurrent=500;							    
								daccount	=(sum_setcurrent*CCSLOPENEW)-CCOFFSETNEW ;//8.0;//CCSLOPE);//5.20);//490);//4.60);//4.70)CCSLOPENEW;
								if(daccount>=4095)
								daccount=4095;
								SPI2BUF		=28672+daccount;								
							}									
