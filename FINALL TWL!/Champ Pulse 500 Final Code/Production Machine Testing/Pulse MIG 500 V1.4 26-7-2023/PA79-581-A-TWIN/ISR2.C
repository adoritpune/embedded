...
void __attribute__((interrupt, no_auto_psv)) _T2Interrupt(void)  //50usec
{
   ...
   else if((weldmode=='G') && (weldcyclestrt==1))
   {
         if(adcreadf==1)
        {
         ...
         if(ACT_CVOLTAGE>=800)//500
         ACT_CVOLTAGE=800;//500
         //MIG_superfast_PID();
         //INT_sck_voltage_pid();
         }   
...
//         sckdaccount = 50;// changed on 24112020
         
         
                  
/*         removed by mahesh         
         if(ACT_CVOLTAGE<=100 && startpcvloop==0)              //(FRQbasevtg+10))
         {
            startpcvloop=1;
         }
         else if(ACT_CVOLTAGE>=140 && startpcvloop==1)
         {
            startpcvloop=0;
         }
...
//      {
//            adcreadoverf=0;
//            //if(SCKVOLTAGE<=720) //640) //4000=100V; 800 count=20V 720=18V 600=15vtG  640=16V CV mode
//            {
//               INT_sck_voltage_pid();
//               //Toggle_pin();
//            }
   ...
   {   
      ...
      DACCSLAT=0;
        if(SPI2STATbits.SPITBF==0)
        {
         ...
         SPI2BUF=28672+sumdaccount;//MAHESH+sckdaccount;//DACREF;//1300;//102A 600
        }   
   }
   }
}
...
void __attribute__((interrupt, no_auto_psv)) _ADCInterrupt(void)// for Each channel adc conversion=5usec; for 3=15usec
{
   ...
   if(weldmode=='G')
   {
      ...
      if(AFcnt>=8)               //15usec*8=120usec
      {
         AFvtg=Total_AFVresult>>3;
         Total_AFVresult=0;
         AFcnt=0;
         adcreadf=1;
      }
    ...
    {    
      if(arrptr<=15) //15//21)//Check 
        ...
        else if(recchksum==uartdata2) // for testing 23112020
        {
            ...
            for(iii=1;iii<=15;iii++) //20//TX_DATA_LENGTH
            {    
         ...
         {               //1
            ...
            if((pstowfstr.setwspeed >= 20) && (pstowfstr.setwspeed <= 220) && (WfEditfg == 1))
            {
         ...
         {               //1
            ...
            if((para.pmigmode.setvoltage != iRxwfVoltage) &&(cWfVChangefg == 0))
            {
	          cWfVChangefg = 1;  
/*	            if(cTestfg == 0)
	            {
