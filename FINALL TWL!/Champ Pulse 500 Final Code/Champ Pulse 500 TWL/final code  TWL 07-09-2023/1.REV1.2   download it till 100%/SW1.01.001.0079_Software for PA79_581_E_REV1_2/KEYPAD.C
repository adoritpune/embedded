...
void key_application(unsigned char keypressed)//@called in main.c ==> while(1){ }
{
	...
	if((keypressed=='A') && (weldmode=='P'))
	{
		//read_array(iProgramLoadLoc);
	}
...
/*	
	...
	TRANS1('R');	
*/	
NumberUpdate(4,5,iProNo);
program_check_from_mem(iProNo);//read_from_external_memory(iProNo);
...
//			}
	...
	else if((keypressed=='D') && (setlcdweldparameter == 0) && (cMacMenufg == 0))
	{
		...
		/*	clrlcd();			
			...
			lcdput(2,0,"SERIAL PORT :"); 
			if(cSerialPortTask == 'H')
			{
	...
	else if((keypressed=='D' && setlcdweldparameter!=0)	&&(cMacMenufg == 0))			//@up keypressed
	{	
			if(lcdpage==1) 		
			{
	...
	else if(keypressed=='E' && setlcdweldparameter!=0)				//@ down keypressed
	{
		if(lcdpage==1) 		
		{
...
//					recall_mode_mem(Recall_location); //"RECALLING DATA";
			...
			else if(setlcdweldparameter==2 && weldmode=='G') //#3rd Press @exit weld parameter
			{
				cMainScreenfg = 1;
				/*
				//++++++++EXIT+PROCESS+++++++++++++++++//
			...
			else if(setlcdweldparameter==2 && weldmode=='P')
			{
				setlcdweldparameter=0;
				lcd_saverecall_ptr=0;
				lcdmigpointer=0;
	...
	else if(keypressed=='G')                //S6 program key
	{
		/*if(weldmode=='M')
		{
			weldmode='T';
