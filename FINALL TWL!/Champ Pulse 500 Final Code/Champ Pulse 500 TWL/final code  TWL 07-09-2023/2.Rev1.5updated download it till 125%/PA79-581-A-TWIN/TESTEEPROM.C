...
/*	
	...
	*/
	...
	if(weldmode=='G')
	{
		...
		if( dia!=zeropteight && dia!=oneptzero && dia!=onepttwo && dia!=oneptsix) //if diameter read from eeprom is wrong
		{
			mat=ms;
			para.migmode.gmaterial=mat;
		...
		if(para.migmode.gmaterial!=ms && para.migmode.gmaterial!=ss && para.migmode.gmaterial!=alu && para.migmode.gmaterial!=msfcaw
	...
	if(weldmode=='P')
	{
		...
		if( pdia!=pzeropteight && pdia!=poneptzero && pdia!=ponepttwo && pdia!=poneptsix)
		{
		...
		if(pgas!=parco2  && pgas!=pco2 && pgas!=par &&  pgas!=paro2)
		{
			pmat=pms;
			para.pmigmode.pmaterial=pmat;
		...
		if(para.pmigmode.pmaterial!=pms      && para.pmigmode.pmaterial!=p71t1 && para.pmigmode.pmaterial!=p180r 
