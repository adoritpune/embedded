...
void MainScreenShow(void) 
{
	...
	if(weldmode=='G')
	{
		...
		lcd_disp_hex_dec(dispautocurrent);
		currupdate();
		if(migUpdateProgfg == 1)
		{
		
		    iProgramLoadLoc=201;
			lcd_disp_hex_dec(201);//program_number);
			programupdate();
			
			
		}	
		else if(migUpdateProgfg == 2)
		{
			
			iProgramLoadLoc=301;
			lcd_disp_hex_dec(301);//program_number);
			programupdate();
		...
		else if(migUpdateProgfg==0)
		{
			if(pidParams2.migweldmode == 1)
			{
		      if(iProgramLoadLoc>=301 && iProgramLoadLoc<=400)
                {
                      iProgramLoadLoc=201;
			...
			else if(pidParams2.migweldmode == 0)
			{
                if(iProgramLoadLoc>=201 && iProgramLoadLoc<=300)
                {
                      iProgramLoadLoc=301;
                      //lcd_disp_hex_dec(iProgramLoadLoc);//program_number);
		              //programupdate();
	...
	if((weldmode=='G'))
	{
		...
		if(iProgramLoadLoc==203 || iProgramLoadLoc==207 || iProgramLoadLoc==211 || iProgramLoadLoc==303 ||iProgramLoadLoc==307 || iProgramLoadLoc==311)
		{
