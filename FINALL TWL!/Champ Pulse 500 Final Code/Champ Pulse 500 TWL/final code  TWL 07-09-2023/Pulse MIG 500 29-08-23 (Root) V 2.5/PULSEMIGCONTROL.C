...
void superfast_PID(void)
{
	 ...
	 sf_kdgain = 20;//8;//15;//8;//5//Recall_location;//8;10;//5;//pidParams.Kd;
...
{
   sf_outputmax=64000; //64000;
   if(pidParams2.migweldmode == 1) // Normal Mode
   {
	   sf_kpgain= 42;
   	   sf_kigain= 2;
       sf_kdgain= 20;
   ...
   else
   {
   	 ...
   	 sf_kpgain = pidParams2.Kp2;//7;//(int)intkp;//42;//30;//intkp;
   	 sf_kigain = pidParams2.Ki2;//7;//(int)intki;//2;//1.5;//intki;pidParams.Ki;
   	 sf_kdgain = pidParams2.Kd2;//17;//(int)intkd;//20;//10;//intkd;pidParams.Kd;
	...
	{    
		//migdaccount = sf_pidoutput;   		// Set Cur 90 V=15.2    kp =10   i=	7  kd= 17 	  7 7 17
		if((para.migmode.setcurrent<=216) && (pidParams2.migweldmode == 0) )
		{
			if(ucCCCVCONTROL==0) 
	         	migdaccount = 	sf_pidoutput;	
	  		else if(ucCCCVCONTROL==1)
				migdaccount = 	uiFristP;			//225;	//uiFristP=(((0.369*para.migmode.setcurrent)+178.2)*1.12);			//225;
			else if(ucCCCVCONTROL==2)		
				migdaccount = 	uiFristDW;				//134;	//uiFristDW=((((0.369*para.migmode.setcurrent)+178.2)/2)*1.12)+25;													//134;
			else if(ucCCCVCONTROL==3)
				migdaccount =  uiSecondP;		// 260;	//uiSecondP=((0.4836*para.migmode.setcurrent)+195.5)*1.12;			//260;
		}
		else
			migdaccount = sf_pidoutput;	
	}
