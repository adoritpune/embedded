...
/*************************************OCV KEY FUNCTIONALITY******************************************/			
			...
			if(weldonf==0)// || dispupdatef==1)
			{
				...
				{
					if(prevhotstart != iHotstartper)
					{
						if((prevhotstart !=iHotstartper) || (dispupdatef==1))
						{
							lcd_disp_hex_dec(iHotstartper);
							arcforceupdate();	
						}
						prevhotstart =iHotstartper;							
					}	
				}		
				...
				//if(presetcurrent!=para.mmamode.setcurrent)
				{
					if(SPI2STATbits.SPITBF==0)
					{
						...
						*/
						...
						if((VOLTAGE<100)&&(para.mmamode.setcurrent >= ANTISTICKCURR))      //For MMA antistick operation //@sbm
						{
						   antistick_f=1;    //flag set
						   sum_setcurrent=ANTISTICKCURR; //Default value 30A(can not be kept less than 20-> weldoff)
						...
						else if((cHotstartfg == 1))
						{
						    ...
						    if(sum_setcurrent>=500)
						    sum_setcurrent=500;
						    if(para.mmamode.setcurrent <= 75)
						    {  
								daccount	=((sum_setcurrent*CCSLOPENEW_1STAG)-CCOFFSETNEW_1STAG);//+Recall_location;								
							}
							else
							{
								daccount	=(sum_setcurrent*CCSLOPENEW)-CCOFFSETNEW ;								
						...
						{
							antistick_f=0;	
							/*if(para.mmamode.setcurrent >= 14 && para.mmamode.setcurrent <= 25 )//122)
							{
								sum_setcurrent=para.mmamode.setcurrent+sck_current_ref;
							    if(sum_setcurrent>=500)
							    sum_setcurrent=500;							    
								daccount	=((sum_setcurrent*10.0)-CCOFFSETNEW_1STAG);//+Recall_location;CCSLOPENEW_1STAG
								if(daccount>=4095)
								daccount=4095;
								SPI2BUF		=28672+daccount;
							
							}	*/					
							 if(para.mmamode.setcurrent <= 51)
							{								
								sum_setcurrent=para.mmamode.setcurrent+sck_current_ref;
							    if(sum_setcurrent>=500)
							    sum_setcurrent=500;							    
								daccount	=((sum_setcurrent*6.5)-CCOFFSETNEW_1STAG);//+Recall_location;CCSLOPENEW_1STAG
								if(daccount>=4095)
								daccount=4095;
								SPI2BUF		=28672+daccount;								
							}
							else if(para.mmamode.setcurrent >= 52 && para.mmamode.setcurrent <= 200 )//122)
							{								
								sum_setcurrent=para.mmamode.setcurrent+sck_current_ref;
							    if(sum_setcurrent>=500)
							    sum_setcurrent=500;							    
								daccount	=((sum_setcurrent*5.6)-CCOFFSETNEW_1STAG);//+Recall_location;CCSLOPENEW_1STAG
								if(daccount>=4095)
								daccount=4095;
								SPI2BUF		=28672+daccount;								
							}
							else
							{
							    ...
							    if(sum_setcurrent>=500)
							    sum_setcurrent=500;							    
								daccount	=(sum_setcurrent*5.3)-CCOFFSETNEW ;//8.0;//CCSLOPE);//5.20);//490);//4.60);//4.70)CCSLOPENEW;
								if(daccount>=4095)
								daccount=4095;
								SPI2BUF		=28672+daccount;								
							}									
