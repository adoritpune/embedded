...
MODEL C== CHAMP MULTI 250 COMPLETED
ADDED TO RECATION CODE IN READ MEMORY AT START FUNCTION TO AVOID GARBAGE DATA
ADDED HALL SENSOR ERROR--- 009
ADDED REF & ACTUAL CURRENT SETTING SAME AS Quality international DUBAI MACHINE
Removed error of ERR-004 in welding when UV/OV error comes during welding. 
FINAL SOFTWARE REV 2.0
CHAMP MULTI 250 		== 22M /MIN
CHAMP MULTI 400/600	== 18M /MIN
...
int main()
{
	...
	{
		vtgmulti		=42.0;
 		curmulti		=680.0;     //716.0; modified as per Quality international
      	motormulti		=200.0;
		Currentfactor	=9.20;
		maxmmacurrent	=399.0;
		maxmigvolts		=40.0;
	...
	else if(mc_model=='B')		//MMA-TIG-MIG-FCAW-500A
	{
		vtgmulti		=56.0;
 		curmulti		=980.0;
     	motormulti		=200.0;
		...
		maxmigwf		=180.0;		// 180.0 changed on 02112020 COFMOW requirement of max speed.
	...
	else if(mc_model=='D')		//MMA-TIG-MIG-FCAW-600A
	{
		vtgmulti		=56.0;
 		curmulti		=980.0;
     	motormulti		=200.0;
...
*/
	...
	DISICNTbits.DISICNT=0x3FFF;
	{
		...
		win1[2]='R';
		win2[0]='2';
		win2[1]='1'; // Rev changed from 2.0 to 2.1 for COFMOW wirespeed change from 180 to 250
		win2[2]=mc_model;
		win1dpcnt=3;
		win2dpcnt=3;
		for(i=0;i<4000;i++)
...
//********************************************************************************
	{
		if(UV_errorf==1 && OV_errorf==0 && TH_errorf==0 && HS_errorf==0)
		{
				win1[0]='E';
				win1[1]='R';
				win1[2]='R';
		...
		else if(UV_errorf==0 && OV_errorf==1 && TH_errorf==0 && HS_errorf==0)
		{
				win1[0]='E';
				win1[1]='R';
				win1[2]='R';
		...
		else if(UV_errorf==0 && OV_errorf==0 && TH_errorf==1 && HS_errorf==0)
		{
				win1[0]='E';
				win1[1]='R';
				win1[2]='R';
		...
		else if(UV_errorf==0 && OV_errorf==0 && TH_errorf==0 && HS_errorf==1)
		{
				win1[0]='E';
				win1[1]='R';
				win1[2]='R';
		...
		//while(Errorf==1); //while((Errorf==1) || (Weldonerrorf==1));
		while((Errorf==1) || (Weldonerrorf==1))
		{
			if(cccvmode==0 && (Weldonerrorf==1))							//to latch the error in MIG welding
			{
...
void genref(unsigned int pwmtm1)
{
	...
	if(setupmode==0)
	{
		result1=((temp1 * 29.49)); //29.49
	}
	else if(setupmode==1)
	{
		result1=((temp1 * 29.49)/10.0); //29.49
	}	
...
//**************
	...
	PR2=2949;			//1472 for 20khz,2949 for 10khz fred	
	//736 for 40khz freq.//2210 for 15Khz /initialise pr2 reg. for freq. // 5898 = 5khz //2949 = 10khz //2100=14khz //2000= 15khz
	...
	for(i=0; i<33; i++)
  	{
		...
		if(para.burnback.time==0xffff || para.burnback.time > 50)
		{
		...
		if(para.fcawws==0xffff || para.fcawws > 250) // 180
		{	
			para.fcawws=0;
		}
		if(para.weld.wfs==0xffff || para.weld.wfs > 250) // 180
		{
			para.weld.wfs=0;
		}
		if(para.fcaw.wfs==0xffff || para.fcaw.wfs > 250) //180
		{
			para.fcaw.wfs=0;
		}
		if(para.migws==0xffff || para.migws > 250) // 180
		{
			para.migws=0;
		}
		...
		if(para.crater.wfs==0xffff || para.crater.wfs > 250) // 250
		{
			para.crater.wfs=0;
		}
		if(para.fcaw.craterwfs==0xffff || para.fcaw.craterwfs > 250) 	//180
		{
		...
		else if(WeldMode==0x03) 	//MIG mode
		{
			...
			TypeU.LEDGROUP[1]	 =0xff;
              	Type.weld	 =0;
                 ;
//###############TAKE PRECAUTIONS TO AVOID THE GARBAGE DATA#########################
			ramdata=TypeU.LEDGROUP[2];
			ramdata1=(ramdata & 0x0f);
			
			if((ramdata1 !=0x0E) && (ramdata1 !=0x0D) && (ramdata1 !=0x0B) && (ramdata1 !=0x07))
			{
					TypeU.LEDGROUP[2]=0xff;    //Type.fcaw=1;
					Type.fe=0;
					Type.zeropteight=0;
					TypeU.LEDGROUP[3]=0xff;
					Type.arco2=0;
	...
	for(i=0; i<33; i++)
  	{
		...
		if(para.crater.wfs==0xffff || para.crater.wfs > 250) //180
		{
			para.crater.wfs=0;
		}
		if(para.fcaw.craterwfs==0xffff || para.fcaw.craterwfs > 250) // 180
...
//################################################################################ //SET Crater parameters
			else if(Type.crater==0 && cyclestrt==0)
			{
				...
				if(Type.mig==0) //|| Type.fcaw==0)
				{
					...
					//para.crater.volts=updatedata(para.crater.volts,min.crater.volts,max.crater.volts,encres); //max.weld.volts
					para.crater.volts=updatedata(para.crater.volts,min.weld.volts,max.weld.volts,encres); //max.weld.volts
...
//#####################################				      
					...
					paravolt=para.crater.volts;
					craterpwmtime1=(paravolt/4.0);//2.0);	//remove para.pwmtime1
					cratervoltage=(((paravolt*vtgmulti)/400.0) + cvoffset);//0.215);//200
//##########################################
...
//#######################################################					
					{
							do
							{
								if(((mc_model=='A')  && autocur > 400) || ((mc_model=='B')  && autocur > 500) || ((mc_model=='C')  && autocur > 250) || ((mc_model=='D')  && autocur > 600))
								para.crater.wfs=para.crater.wfs-1;

								temp1=para.crater.wfs;
								fwirespeed=temp1*0.101;//30;
								fpwmtime=fwirespeed*3.0;
								//para.pwmtime=fpwmtime;
								craterpwmtime=fpwmtime;
								if((fwirespeed!=prevfws) || keychange1==1)
								{
									...
									if(Type.automan==0)
									{
										...
										para.crater.volts=((autovolt - (cvoffset*10))* 40.0)/vtgmulti; //20
									}
								}	
								prevfws=fwirespeed;	
							}
							while(((mc_model=='A')  && autocur > 400) || ((mc_model=='B')  && autocur > 500) || ((mc_model=='C')  && autocur > 250) || ((mc_model=='D')  && autocur > 600));
						}
/*
						{	
							if(cratervoltage >=maxmigvolts) //40.0
							errflag1='O';
							else if(cratervoltage <=15.0)
							errflag1='U';
							else
							errflag1='N';
						}
...
//#########################################################
					...
					*/
					...
					if(errflag1=='N' || (errflag1=='U' && encres=='+') || (errflag1=='O' && encres=='-')) 
					//para.crater.volts=updatedata(para.crater.volts,min.crater.volts,max.crater.volts,encres); //max.weld.volts
					para.fcaw.cratervolts=updatedata(para.fcaw.cratervolts,min.weld.volts,max.weld.volts,encres); //max.weld.volts
					//cratervoltage=(para.crater.volts*cratervoltfactor);//0.27);//0.22);
//#####################################				      
					//para.weld.volts=updatedata(para.weld.volts,min.weld.volts,max.weld.volts,encres);
					paravolt=para.fcaw.cratervolts;//para.crater.volts;
					craterpwmtime1=(paravolt/4.0);//2.0);	//remove para.pwmtime1
					cratervoltage=(((paravolt*vtgmulti)/400.0) + cvoffset);//0.215);//200
//##########################################
					if(errflag2=='N' || (errflag2=='U' && encres1=='+') || (errflag2=='O' && encres1=='-'))
					para.fcaw.craterwfs=updatedata(para.fcaw.craterwfs,min.crater.wfs,maxmigwf,encres1);
//#######################################################					
					{
							do
							{
								if(((mc_model=='A')  && autocur > 400) || ((mc_model=='B')  && autocur > 500) || ((mc_model=='C')  && autocur > 250) || ((mc_model=='D')  && autocur > 600))
								para.fcaw.craterwfs=para.fcaw.craterwfs-1;
								...
								if((fwirespeed!=prevfws) || keychange1==1 )
								{
									...
									if(Type.automan==0)
									{
										...
										voltage2=findmatindex2((wirespeed+1));
										autovolt=voltage1+((fwirespeed-wirespeed)*(voltage2-voltage1));
										para.fcaw.cratervolts=((autovolt - (cvoffset*10))* 40.0) /vtgmulti; //20
									}
								}
						 		prevfws=fwirespeed;
							}
...
void torchaction()
{
		 ...
		 if(Type.twot==0)
		{
			switch(oper)
			{
				...
				{
					oper=weld;
					if(autoinch==0 && Motorerrf==0 && Weldonerrorf==0)
					{
						ARCONLAT=1;
						genref(para.pwmtime1);								//generate reference
						drive_motor(para.pwmtime);
				...
				case weld:
				{
					...
					if(autoinch==0 && Motorerrf==0 && Weldonerrorf==0)
					{
						...
						else if(dummytorch==1 && torchrelease==1 && cratersetup==0)// && Type.crateron==0)//TORCH PRESSED AGAIN CRATER FUNCTION
						{
							...
							{
								genref(craterpwmtime1);//(para.crater.volts/2.0);								//generate reference
								drive_motor(craterpwmtime);//((para.crater.wfs*0.101)*3.0);
							}
							else if(Type.mig==1 && Type.fcaw==0)
							{
								genref(craterpwmtime1);//para.fcaw.cratervolts/2.0);							//generate reference
								drive_motor(craterpwmtime);//((para.fcaw.craterwfs*0.101)*3.0);
							}
							cratersetup=1;
						}
