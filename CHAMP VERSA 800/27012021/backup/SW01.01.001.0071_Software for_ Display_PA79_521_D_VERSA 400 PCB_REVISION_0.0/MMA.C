        ...
        if((trip_flag==0)&&(imtrip_flag==0))          //IF TRIP OFF
         {
			...
			/*
				...
				//else 
				if((hotf==0) && (mma_weldvoltage<=600) && (arcforce!=0))         //Arc detection
				{
					if(setcurrent<=200)
					{
						//fnewcurrent=fsetcurrent + (((600.0-fweldvoltage) * fsetcurrent *farcforce )/(600.0 * 100.0));
						setcurrent=setcurrent + (((600-mma_weldvoltage) * fsetcurrent *farcforce )/(600 * 100));
				 	}
				} 	
				 	else
				 	{
				...
				else if(0)//hotf==0)                                //no hotstart needed
				{
				    //fnewcurrent=fsetcurrent;
				}
				*/			
...
//---------------------------------------
		...
		{
			DACCSLAT=0;
			dac_counts=MMACCLookup(setcurrent);//*MMACCSLOPE) +  MMACCOFFSET;	//
