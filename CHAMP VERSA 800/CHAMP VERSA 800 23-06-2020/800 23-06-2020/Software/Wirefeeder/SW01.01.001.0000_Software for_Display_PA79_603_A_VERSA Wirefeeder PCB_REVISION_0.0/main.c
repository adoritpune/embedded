...
void interrupt ISR_Function(void)
{
    if(INTCONbits.TMR0IF==1)      //INTERRRUPT @ 2MSEC
    {
        ...
        if(motor_acceleratefg==1)
        {
            up_count_motor++;
            if(up_count_motor > 1000)
            {
                up_count_motor=0;
                motor_acceleratefg=0;
                motor_continuefg=1;
            }
        }
...
//-------------------
        ...
        if(count_1msec >= 10)//20)
        {
            ...
            else
            {
                ...
                if(tans==1)
                {
                    ...
                    if(TORCH=='P')
                    {
                        if((motor_acceleratefg==0) && (motor_continuefg==0))
                        {
                            motor_acceleratefg=1;
                            up_count_motor=0;
                        }
    ...
    {    
        ...
        { 
            ...
            {
                pcount=0;
                avgpperiod=Total_Period>>4; //LHS 3 TIMES = DIVIDE / 8 & LS 1 TIMES, 65535 COUNTS =32768USEC
    ...
    if(PIE1bits.RCIE & PIR1bits.RCIF) //&& (serialintf==1))
    {
        ...
        //if(txonf==0)
        {
            ...
            else if(startptr1==1 && statusptr1==0)
            {
                ...
                else if(recchksum==rxdata)// /*if(arrptr>=RECEIVE_STRING_LENGTH+1)//*/ changed on 03122019
                //else if(recchksum==rxdata)
                {
                    
//                    recchksum = 0;
//                    for(arrptr = 0; arrptr <= RECEIVE_STRING_LENGTH; ++arrptr)
//                    {
//                        recchksum=recchksum+rec_arr[arrptr];
//                    }
//                    iTest = recchksum;// rxdata;
//                    arrptr++;
//
//                    iTest = recchksum;// rxdata;
                    //Type.gason=1;
                  //  if(1)//recchksum==rxdata)
                //    {
                        comm_error_count=0;
                        if( cEditfg == 0)
                        {
                        ...
                        //Actual_amp = scaled_baseval2;//setpfreq;
                        //Actual_vtg = Fltpfreq;
                        ...
                        if((psstatus.weldoncyclestrt == 0)&&(cWeldStopfg == 0))
                      {
                        Type.wirespeed=1;
                        Type.amp=0;
                       ...
                       //if(psstatus.migpmigmode==0)      //changed by sayali
                        {
                            Type.arclength=0;
                            Type.vtg      =1;
                        }
    ...
    //--------------
    //                    Actual_amp = recchksum;//scaled_baseval2;//setpfreq;
    //                    Actual_vtg = rxdata;//Fltpfreq;
    //--------------
                        startptr1=0;
                        arrptr=0;
                        recchksum=0;
                        recstatus=0;
                        statusptr1=0;
                        transmitonf=1;       //data sent only when data is received
                        onesecf=1;          //DATA WILL BE SENt ONLY WHEN DATA RECEIVED
                   // }
                }    
                else
                {
  ...
  /**************************Encoder read start here*************************************************/
        ...
        {    
            ...
            {    
                mili2_100++;
                encod2=SCANENCODER2();
                if(Type.vtg==1)// (psstatus.migpmigmode==0)  //mig mode //changed by sayali
                {
                    if(encod2=='+')
                    {
                ...
                /*
                else if(Type.arclength==1) //sayali
                {
                    if(encod2=='+')
                    {
...
/******************************When weld is OFF********************************************************/
      if(psstatus.weldoncyclestrt!=1)//(TORCH=='X')
        {
            ...
            if(((parclength!=pstowfstr.arclength) || dispupdatef==1) && (Type.arclength==1))
            {
                ...
                Typedp.digitdp5    =0;
                Typedp.digitdp6    =0;
                Type.arclength     =1;  //changed from 1 to 0 30-9-19
                Type.vtg           =0;
                Type.sec           =0;
           ...
           */
            //if(inchkeyon==0)
            if((psstatus.gasoncommand!=1)&&(gaschkfg == 0))
                GAS_RLY         =0;
            ...
            if(psstatus.motoroncommand==1)
            {
               if(psstatus.creepspeedon==1)
               {
                 ...
                 //   newsetwirespeed=(setwirespeed>>1);
                    ...
                    if(newsetwirespeed<=20)// changed on 28112019
                      newsetwirespeed=20;//setwirespeed;

                //    else if(newsetwirespeed>=60)
                //       newsetwirespeed=60;
                    
                     Update_PWM(newsetwirespeed*MTR_VTG_CONSTANT);// commented on 28112019
 /*
                     if(newsetwirespeed <= 60)
                    {
                        Update_PWM(newsetwirespeed*MTR_VTG_LOW_CONST);
                    }else
                    {
                        Update_PWM(newsetwirespeed*MTR_VTG_HIGH_CONST);
                    }
*/
                     TRISMOTOR_TRIP11=0;
                     MTRFB=1;
               ...
               if(up_count_motor==0)
               {
                   if((newsetwirespeed >= 20) && (newsetwirespeed <= 60))
                   newsetwirespeed=(setwirespeed*14) / 10;
                   else if((newsetwirespeed > 60) && (newsetwirespeed <= 120))
                   newsetwirespeed=(setwirespeed*12) / 10;
               }
               ...
               */
               if ((newsetwirespeed >= 20) && (newsetwirespeed <= 220))
               {
                   setpfreq= Wirespeed(newsetwirespeed);//newsetwirespeed*MTR_FREQ_CONSTANT_3;
               }
               
                Hi_setppwm=(newsetwirespeed+rel_wirespeed)*MTR_VTG_CONSTANT;
                Lo_setppwm=(newsetwirespeed-rel_wirespeed)*MTR_VTG_CONSTANT;//(setwirespeed-10));//-0.044;
            }
            else if(psstatus.motoroncommand!=1)
            {
                Update_PWM(setwirespeed*0);
                openloopcnt =0;
                closeloopf  =0;
...
//            Type.wirespeed=0;
            Type.amp=0;
            Type.thickness=0;// blocked on 01062020
            Type.wirespeed=1;// transfer to wirespeed
            TypeDP.DPGROUP[0]=0x00;
            Typedp.digitdp2    =1;
            Typedp.digitdp5    =1;
            //Send_Byte('a');
...
unsigned int Wirespeed(unsigned int wireSpeed)
{
   unsigned iFreq;
    if((wireSpeed >= 20)&&(wireSpeed <= 29))
        iFreq=wireSpeed*MTR_FREQ_CONSTANT_3;
   else if((wireSpeed >= 30)&&(wireSpeed <= 39))
        iFreq=wireSpeed*MTR_FREQ_CONSTANT_2;
   else if((wireSpeed >= 40)&&(wireSpeed <= 59))
        iFreq=wireSpeed*MTR_FREQ_CONSTANT_1;
   else if((wireSpeed >= 60)&&(wireSpeed <= 110))
        iFreq=wireSpeed*MTR_FREQ_CONSTANT_3;
   else if((wireSpeed >= 111)&&(wireSpeed <= 119))
        iFreq=wireSpeed*MTR_FREQ_CONSTANT_4;
   else if((wireSpeed >= 120)&&(wireSpeed <= 220))
        iFreq=wireSpeed*MTR_FREQ_CONSTANT_5;
