              ...
              // if(mma_init==1)         	//MMA INITIALISATION OVER
               	 {
						...
						if(arcforcemodef==0 && dispcalmodef==0)  //@Display set current
						{
							...
							if(weldon==0)
							{
								...
								if(((pweldvoltage != mma_weldvoltage)  && disp_flag==1) || Disp_updatef==1)                   
								{
								   	 disp_hex_dec_volt(600.0);	 //DISPLAY VOLTAGE
				...
				if((arcforce !=0) && (setcurrent <= 600))
				{
					if((mma_weldvoltage > 100.0) && (mma_weldvoltage <= 600.0))
					{												
						iArcforceCurr=setcurrent+(setcurrent*((600.0-mma_weldvoltage)/600.0)*(arcforce/100.0));
						if(iArcforceCurr >= 650)
						{
							iArcforceCurr=650;
						}
						iPrevcurr=iArcforceCurr;									 		
				 	}					   		
				}
				else 
				{
					iPrevcurr=setcurrent;				
				}
...
//---------------------------------------
		...
		{
			DACCSLAT=0;
			dac_counts=MMACCLookup(iPrevcurr);//*MMACCSLOPE) +  MMACCOFFSET;	//
			/*if(fnewcurrent <=105.0)
			{
				dac_counts = (fnewcurrent*MMACCSLOPE100)+MMACCOFFSET100;
			}
			else
			{
				dac_counts = (fnewcurrent*MMACCSLOPE)+MMACCOFFSET;
			}
			*/	
...
//---------------------------------------			 
			 ...
			 {				
    			...
    			if((pweldvoltage != weldvoltage) && disp_flag==1)  //AFTER EVERY 1 SEC
    	  		{
    				disp_hex_dec_volt(600.0);      //weldvoltage //DISPLAY ARC VOLTAGE	
...
//-------------------------------------
				if(hotf==1) 
			   	{						
					if(setcurrent <=550)
					{						   	
						iHotstartcurr=setcurrent+((setcurrent*HOTSTRT_PERCNT_1)/100.0);  
						if(iHotstartcurr>=650)
						{
							iHotstartcurr=650;
						}
						iPrevcurr=iHotstartcurr;

						if(SPI2STATbits.SPITBF==0)      //for versa 800
						{
							DACCSLAT=0;
							dac_counts=MMACCLookup(iHotstartcurr);	
							SPI2BUF=28672+dac_counts;								
						}					 					 	
					}
					else
					{
						...
						{
							DACCSLAT=0;
							dac_counts=MMACCLookup(iPrevcurr);	
							SPI2BUF=28672+dac_counts;								
						}  
					}
				}
...
unsigned int MMACCLookup(unsigned int iCurrent)
{
	...
	{
		iDacref = iCurrent*MMACCSLOPE100 +  MMACCOFFSET100;	
	}
