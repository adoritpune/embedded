...
///////////////////////////////////////////////////////////////////////////////////////

 {
	       if(pidvaluesf==0)
		  {
			   ...
			   else if(ENCODERSW==0 && keyrelease==1)              //SWITCH PRESSED
			   {
					//arcforcemodef=0;
					Disp_updatef=1;
					keyrelease=0;
		  ...
		  else  if(pidvaluesf==1)
		  {
				PIDvalues();                          //cal mode function is in ISR file
		  } 
		  ...
		  if(pidvaluesf==0)
		  {
			if(ENCODERSW==0)          //encoder switch is pressed for 5 sec then enters in calibration mode
			{
				...
				if(dvalcnt>=5000)    //5sec
				{
					pidvaluesf=1;       //entry in calibration mode
					//setupcalmode=1;
					//arcforcemodef=0;
					Disp_updatef=1;
					keyrelease=0;
			...
			{
				dvalcnt=0;
				pidvaluesf=0; //calmodef=0;
				//setupcalmode=0;
				dispidvlauesf=0;   //dispcalmodef=0;
			}
		 }
           ...
           /*if(weldcurrent >=300)
           {
		   	...
		   	else
		   	{
				...
				if(weldcurrent >=offset)
				{
					...
					else if(weldcurrent>=590 && weldcurrent<= 610)
