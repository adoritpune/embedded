		...
		else if(Torchmode=='4')
		{
			switch(oper)
			{
				case pre:
				{
					...
					if(count_preflow>=0 && OCV_FLAGLAT==0)
					{
						OCV_FLAGLAT	=1;          		//OCV ON
						ARCONLAT	=1;				//ARCON OUTPUT SIGNAL TO PWM PCB
						CCCVLAT		=CVMODE;		//CV MODE SELECTION IN PWM CARD
...
{
	sf_scalefactor=6;
 	sf_outputmax=32000; //64000;	
	sf_kpgain=iKp;  //25; //150;//25;//75;//64;
	sf_kigain=iKi;  //10; //2
	sf_kdgain=iKd; //10; //8
	//dt=0.01; //iTime/100;
	//epsilon=0.01; //iEp/100;
	mult_factor=10; //20;
	...
	if((rset_wirespeed >= 40) && (rset_wirespeed <= 20))
	{
		if(sf_error==0)
		{
			sf_accumulator=0;
	...
	else if((rset_wirespeed >= 20) && (rset_wirespeed < 40))
	{
		if(sf_error==0)
		{
			sf_accumulator=0;
...
*/	
	...
	if((MIGweldon_detectf==0) && (sf_error<0))
	{
    	migdaccount1=200;
	}	  
	else
...
sf_scalefactorr=6;
sf_outputmaxx=32000; //64000;	

//tPID fooPID;


fractional fooPIDCoeffs[3] = {0,0,0};
	
fooPID.abcCoefficients = &abcCoefficient[3];
fooPID.controlHistory = &controlHistory[3];

PIDInit(&fooPID);

fooPIDCoeffs[0] = Q15(iKp);
fooPIDCoeffs[1] = Q15(iKi);
fooPIDCoeffs[2] = Q15(iKd);
	
PIDCoeffCalc(&fooPIDCoeffs[3], &fooPID);

fooPID.controlReference = Q15(rsetvoltage);
fooPID.measuredOutput = Q15(actual_voltage);
...
migdaccount2 = PID;
/*
	if(migdaccount>=sf_outputmaxx)
	{
	   migdaccount=sf_outputmaxx;	
	}
	else if(migdaccount<=sf_outputminn)
	{
	   migdaccount=sf_outputminn;
	}
	
	migdaccount=PID>>sf_scalefactorr;    //sf_pidoutput is used in isr
	...
	else
	{
		if(1)//sf_error>=0)  
	    {
		    if(migdaccount<=iLowpidvalue) //20) 
			migdaccount=iLowpidvalue; //20;
			if(migdaccount>=500)
			migdaccount=500;
	    	migdaccount2=migdaccount;
	    }
	}
