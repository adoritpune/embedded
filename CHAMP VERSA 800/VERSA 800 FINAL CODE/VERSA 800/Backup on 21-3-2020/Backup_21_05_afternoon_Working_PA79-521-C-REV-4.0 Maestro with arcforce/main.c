...
{   
	...
	//---------------    
    iKp=EEPROM_Read(20*2);//250;
    iKi=EEPROM_Read(22*2);;//10;
    ...
    if(iKd>= 200)
    {
    	...
    	iKd=20;	
    	iMul = 50;
		EEPROM_Erase_Write(20*2,iKp);
		EEPROM_Erase_Write(22*2,iKi);
    ...
    //weldvoltage=22;
		...
		for(i = 0; i <= 1000; i++)
			...
			*/
//	trans_head('A');
communication.creep_speed = 20; //35//100;
iMigOffset = 700;
iLowpidvalue = 20;
imfvalue = 1;
   	while(1)
			...
			{ 
	      	...
	      	else if(process_flag==MIGMODE)
	      	{
				...
				if(cpidmode==7 && weldon==0) //@Display set Arcforce
				{
	      		...
	      		mig();
				if(dispweldcurrent>= 56) //46)   //45)) //74) //40) //74)  
//					if(weldcurrent >= 20)
					{
						MIGweldon_detectf=1;
						psstatus.creepspeedon=0;
					}	
	      		if(transmit2wf_f==1)//trans_flag==1 && mig_trans_f==1 ) //trans_flag in MIG1
	      		{
	      		   transmittowf();
	      		   transmit2wf_f=0;
	      		   //mig_trans_f=0;
...
//  ADCSSL = 0x400F;            // SELECT AN0,AN1,AN2,AN3, AN14 FOR INPUT SCANNING
	...
	ADCON2bits.SMPI = 2; //5;//m3;	// interrupt after every 6th sample one sample from each channel 	24012020
