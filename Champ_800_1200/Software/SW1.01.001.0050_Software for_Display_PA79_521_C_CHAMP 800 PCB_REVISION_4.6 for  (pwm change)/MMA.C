...
{   
        if(mma_flag==0)
	  {
		...
		if(memdata >= iSetcurrbuff)
			memdata=iSetcurrbuff;
		setcurrent=memdata;
		memdata=EEPROM_Read(3*2);
		if(memdata >= 100)//iSetmincurrbuff)
			memdata=100;//iSetmincurrbuff;
		arcforce=memdata;
		memdata=EEPROM_Read(6*2);
			...
			CCCVLAT=1;				//CCCV MODE SELECTION IN PWM CARD  cc mode
              ...
              // if(mma_init==1)         	//MMA INITIALISATION OVER
					...
					TypeDP.DPGROUP[0]=0x00;
						...
						{
							if(cLockkeyfoundfg == 0 && cCalikeyfoundfg == 0)
							{
								Typedp.digitdp7	=0;
								CURR[0]='A';
								CURR[1]='R';
							...
							else if(cCalikeyfoundfg==1) 
							{
								...
								CURR[2]='U';
								CURR[3]='R'; 
								disp_hex_dec_volt(iCalicnt);
								cCaliFoundfg=1;
								fCalicnt=iCalicnt/100;
								//cCaliEditfg=1;
								if(EEpromWrfg == 0)
								{
									EEPROM_Erase_Write((6*2),iCalicnt);
									EEpromWrfg = 1;								
								}
							}
							else
							{
								if(cLockfg)
								{
									...
									VOLT[3]='E';
//---------------------------------
//here we are storing the current limit lock as per set current 
								if(EEpromWrfg == 0)
								{
									//EEPROM_Erase_Write((8*2),setcurrent);
									iSetAmpLimit = setcurrent;
									EEpromWrfg = 1;								
								}
//---------------------------------																		
								}
								else
								{
									...
									VOLT[3]='E';
//---------------------------------
//here we are storing the current limit as per selected model 
								if(EEpromWrfg == 0)
								{
									EEPROM_Erase_Write((8*2),iSetcurrbuff);
									iSetAmpLimit = iSetcurrbuff;
									EEpromWrfg = 1;
								}
//---------------------------------																		
								}
...
/*						else if(arcforcemodef==0  && dispcalmodef==0 && weldon==0 && displockmodefg==1)
						{
							CURR[0]='L';
							CURR[1]='O';
							CURR[2]='C';
						...
						else if(arcforcemodef==0  && dispcalmodef==0 && weldon==0 && dispunlockmodefg==1)
						{
							CURR[0]='U';
							CURR[1]='n';
							CURR[2]='L';
				...
				if(fsetcurrent<550)
				{
				fnewcurrent=fsetcurrent+(fsetcurrent*HOTSTRT_PERCNT_1)/100.0;  	
				}
				else
				{	
				...
				else if(fweldvoltage<=600.0)
				{
					if(setcurrent <= 600) //200 1/03/2021							//changed on 10/01/2020
					{												//arc force is active onl if setcurrent is below 200	
			    ...
			    if(fnewcurrent>=iSetcurrbuff)
				fnewcurrent=iSetcurrbuff;
					
				PWMcount=405.0+(fnewcurrent*2.180);	//modified for maestro 1200
				/*
				if(1)//iSetAmpLimit == 800)
				{
					PWMcount=375.0+(fnewcurrent*2.180);	//modified for champ 800 for 60A
				}
				else if(iSetAmpLimit ==1200)
				{
			 ...
			 if(weldon==1)
			 {
				...
				if((pdispweldcurrent != weldcurrent) && disp_flag==1  )  //AFTER EVERY 1 SEC
				{
				...
				PWMcount=427.0+(newcurrent*2.12);	   //modified for maestro 1200
				if(PWMcount >= 2940)
				PWMcount=2940;
				OC1RS= PWMcount;	
				psetcurrent = setcurrent;	
				...
				Disp_updatef=1; 
		  	...
		  	else if(scalmode==1)
		  		{
					...
					PWMcount=375.0+(setcurrent*2.180);	   //modified for maestro 1200
					if(PWMcount >= 2940)
					PWMcount=2940;
					OC1RS= PWMcount;	
			    		psetcurrent = setcurrent;	
