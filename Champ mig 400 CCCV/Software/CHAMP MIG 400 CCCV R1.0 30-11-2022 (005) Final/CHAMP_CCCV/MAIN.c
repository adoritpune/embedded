...
/*********************EEPRROM READ AT START=end	*******************/
	...
	mig_Kp = 42;//35;//70;//60;//50;//10;//5;			20/70/2/1/5	//10		//1/10/1/4/2
	mig_Ki = 2;//2;//1;//3;//70;//8;//45;//8;//70;				//45
	mig_Kd = 20;//2;//80;//50;//100;//95;//1;//2;					//2
	mig_Ka = 95;//0;//2;						//2
	...
	{	
		...
		if(DISPavgf==1)
		{
			...
			VOLTAGE	=(DISPvtg>>2)*0.96; //0.96;	//0.95	(adcvtg*10.0)/40.0;//37.0
...
void ADC_Reference(void)
{
    ...
    {   
		...
		//IRcurr = IRcurr*200.0/400;  // for 20v wire speed 
        IRcurr = IRcurr*200.0/350;   
          
		//IRcurr = IRcurr*240.0/140;  // for 24v (240) wire speed 
		//IRcurr = IRcurr-54;
	
        if(IRcurr >= 662)//PPC 240
		{
			IRcurr = 662;
		}
		else if(IRcurr <= 1)//PPC 240
		{
			IRcurr = 1;
		}	
	
		iWireSpeed=IRcurr;//expon_lookup[IRcurr];
		
		if(iWireSpeed <= 20)
			iWireSpeed = 20;
		if(iWireSpeed >= 240)
			iWireSpeed = 240;				
		if(iWireFeeder_error >= 450)     //450,550,530.520,490
		{
			WireFeeder_errorf=1;
		}
		else
