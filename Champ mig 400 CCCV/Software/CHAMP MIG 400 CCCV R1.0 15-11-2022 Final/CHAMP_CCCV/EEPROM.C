...
void read_current_status(void)
{
		...
		memoryzone='X';
		eeprom_location=MIG_PARA_MAX_SIZE*0*0;												//@Offset for MIG ParaMeter 
        EEPROM_struct_mig.EEPROM_struct_u=EEPROM_Read_for_SaveRecall((eeprom_location+0*2));     //0 eeprom structure
		weldmode			=EEPROM_Read_for_SaveRecall((eeprom_location+0*2));  	//Mode Read
...
/********************************Machine Serial Number End**************************/
	...
	else if(weldmode=='M')
	{
		...
		prev_weldmode=weldmode;
		prev_eeprom_tig_struct=EEPROM_struct_tig.EEPROM_struct_ut;
        iOffset			=EEPROM_Read_for_SaveRecall((eeprom_location+2*1));  	
...
{
	unsigned long int eeprom_location;
	unsigned long int eeprom_location_mode;
	
	eeprom_location_mode=MIG_PARA_MAX_SIZE*0*0;
	iWeldcount++;
 /********************************Arc On Time**************************/
...
/********************************Arc On Time**************************/
	...
	if(weldmode=='G')  //@ mig welding mode
	{
		...
		{
			EEPROM_Erase_Write_for_SaveRecall((eeprom_location_mode+0*2),weldmode);  
		    prev_weldmode=weldmode;	
	...
	else if(weldmode=='M')
	{
	   eeprom_location=MIG_PARA_MAX_SIZE*2*2;
        ...
        if(read_iOffset!=iOffset)
		{
		EEPROM_Erase_Write_for_SaveRecall((eeprom_location+2*1),iOffset);  		 //2 slope 
		read_iOffset=iOffset;
		}
...
void check_eeprom_values(void)
{
	...
	eeprom_location=MIG_PARA_MAX_SIZE*0*0;   //For Default State Selection
	if(weldmode!='G' && weldmode!='M' && weldmode!='T')
	{
		weldmode='G';
		EEPROM_Erase_Write_for_SaveRecall((eeprom_location+0*2),weldmode);  
		prev_weldmode=weldmode;		
	}
	...
	if(weldmode=='G')
	{
		...
		///////////////////////////////////////////////Default flag Slope Read
		eeprom_location=MIG_PARA_MAX_SIZE*2*2;
         
        if((iOffset <= 0)|| (iOffset >= 700))
		{
			iOffset=630;
			EEPROM_Erase_Write_for_SaveRecall((eeprom_location+2*1),iOffset);  		 			//2 preflow
			read_iOffset=iOffset; 
		}
		
		///////////////////////////////////////////////Mode Selection///////////////////////////////////////
		eeprom_location=MIG_PARA_MAX_SIZE*0*2;
		weldmode='G';
		EEPROM_Erase_Write_for_SaveRecall((eeprom_location+0*2),weldmode);  
		prev_weldmode=weldmode;	
