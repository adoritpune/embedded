	  ...
	  if(VRDTimecnt_1s >= 10)
	   {
	       ...
	       if(iVRD_Time != 0) 
	       {
	           if((VOLTAGE > 600) && (weldonf == 0))
	           {
	               ivrdcnt++;
	               if((ivrdcnt >= (iVRD_Time)) && (cVRD_fg == 0))                 
	               {                   
	                   //SHDNLAT = 1;		//OCV
	                   //TRIP_LEDLAT = 0;
	                   ivrdcnt = 0;
	       ...
	       if(cVRD_fg == 1) 
	       {
	           if((((iActualVRDvtgON*90)/100) > VOLTAGE) && (cRecordVRDONvtg == 0))
	           {
	               ivrdcnt = 0;
	               cVRD_fg = 0;
	               //SHDNLAT = 1;
...
/**************************VRD********************************************************/
	...
	if(0)//weldmode == 'M') 
	{
	    if((VRD_SWITCH == 0) && (cVRDSwitchfg == 0))
	    {
	        iVRDSwitchcnt++;            
	        if(iVRDSwitchcnt >= 5)
	        {	           	            
	            TRIP_LEDLAT = 0;
	            cVRDSwitchfg = 1;
                iVRD_Time = VRD_TIME_MAX;
				//MPWMLAT = 1;
	        }                
	    }
	    if((VRD_SWITCH == 1) && (cVRDSwitchfg == 1))
	    {
	        iVRDSwitchcnt++;            
	        if(iVRDSwitchcnt >= 5)
	        {	         
...
*/  
/*********************TIG Torch switch read*********************************************/  
/*     
	if(TORCHIP ==0 && (weldmode=='T') )
	{ 
		dVal_torch0=0;
		if(++dVal_torch1>50)
		{
	...
	if((weldmode=='G'))
	{
		...
		{ 
			...
			{	
				...
				else if((Type.fourt == 1) &&(cTorchStatus == 0))
				{
					cTorchStatus = 1;	
				}		
				GAS_RELAYLAT = 1;
		...
		{ 
			...
			if(++dValm_torch0 > TORCH_DETECT_DB);
			{
				...
				else if((cTestMotorStopfg == 1) && (Type.fourt == 1) && (cTorchStatus == 1))
				{		
					cTestMotorStopfg = 0;					
					cTorchStatus = 2;					
					TORCH_SW = 'T';					
				}	
				else if((cTestMotorStopfg == 1) && (Type.fourt == 1) && (cTorchStatus == 2))
				{
					OC4RS=0;					
					cTestMotorStopfg = 0;
					cMotorStopfg=1;	
...
//--------------------------------------------------------------------------------
      if(weldmode == 'G')
      {                  
	      if((cBKeyLockfg==1) && (cGasChkMenufg == 0) && (cMIGParaMenufg == 0) && (cGasAndWeldParalockfg == 0) && (cVoltageoffsetmenu == 0))                    //SWITCH PRESSED
	      {               
	          iGasChkMenuModecnt++;
	      ...
	      else if((cBKeyLockfg==1) && (cGasChkMenufg == 1) && (cMIGParaMenufg == 0))
	      {
	          ...
	          if(iGasChkMenuModecnt >= 2000) 		//5000
	          {       
	      ...
	      else if((cBKeyLockfg==1) && (cGasChkMenufg == 1) && (cMIGParaMenufg == 1) && (cVoltageoffsetfg==0))
	      {
	          ...
	          if(iGasChkMenuModecnt >= 5000) 		//5000 PPC for volage offset
	          {       
		          iGasChkMenuModecnt=0;
	              cGasChkMenufg = 0;                  
	              cMIGParaMenufg = 0;
                  cVoltageoffsetmenu =1;   
	              cVoltageoffsetfg = 1;                          
	          }  	       
	      }
